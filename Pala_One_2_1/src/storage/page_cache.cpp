...
// `openAndValidateCache` does in one step. The raw form exists for
// `readCacheProgress` + `pageCacheCoversOffset`, which deliberately probe
// the header before the book file is open (and therefore can't yet know
// the actual book size).
...
void writeFreshCache(const String& path, size_t fileSize,
  ...
  f.write(reinterpret_cast<const uint8_t*>(offsets), count16 * sizeof(uint32_t));
  f.close();
}
...
bool appendPagesToCache(const String& path, size_t fileSize,
  ...
  if (f.write(reinterpret_cast<const uint8_t*>(newOffsets), n * sizeof(uint32_t)) != n * sizeof(uint32_t)) {
    f.close();
    return false;
  }
  ...
  if (f.write(reinterpret_cast<const uint8_t*>(&newCount), sizeof(newCount)) != sizeof(newCount)) { f.close(); return false; }
  f.flush();
  f.close();
  return true;
}
...
bool readCacheProgress(const String& path, const PageCacheLayout& layout,
  ...
  uint32_t off = 0;
  if (f.read(reinterpret_cast<uint8_t*>(&off), sizeof(off)) != sizeof(off)) { f.close(); return false; }
  f.close();
...
bool pageCacheCoversOffset(const String& path, const PageCacheLayout& layout,
  ...
  uint32_t storedFileSize = 0;
  if (!readCacheProgress(path, layout, &count, &lastOffset, &storedFileSize)) {
    return false;  // no cache, or layout mismatch — pagination required
  }
  return lastOffset >= targetOffset;
}
