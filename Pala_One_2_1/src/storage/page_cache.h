...
// Lightweight "how far have we cached?" probe used by the idle paginator.
// Returns true if the cache file exists with matching magic + layout +
// non-zero count; `*outCount` and `*outLastOffset` carry the count and the
// offset at the highest cached page, and `*outStoredFileSize` returns the
// book-file size recorded in the cache header at save time (for the caller
// to compare against the current book size — done in a separate step so
// the paginator can decide "no more work needed" without yet having opened
// the book to know its size). Returns false on absence or layout mismatch.
