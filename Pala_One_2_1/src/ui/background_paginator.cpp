#include "src/ui/background_paginator.h"
...
// Grep "BackgroundPaginator" on serial to follow what the paginator's doing.
#define BPLOG(fmt, ...) Serial.printf("[BackgroundPaginator] " fmt "\n", ##__VA_ARGS__)
...
// Sized for the loading-screen path (~1s budgets at 240 MHz → ~80 pages),
// with headroom; the background paginator's smaller budget never comes
// close. If a future budget pushes through it, the loop breaks early,
// the single end-of-tick flush captures what we have, and the next tick
// continues. At 4 bytes/entry this costs 512 bytes of stack.
static constexpr int kMaxPagesPerTick = 128;
...
bool tick(uint32_t budgetMs) {
  ...
  if (layoutMatch) {
    if (paginatedFarEnough(cachedLastOffset, cachedCount, targetOffset, targetPage)
      ...
      BPLOG("already caught up: book=%s pages=%u lastOffset=%u target=%u",
  ...
  if (!bookFile) {
    ...
    BPLOG("book gone, clearing tracking: %s", path.c_str());
  ...
  BPLOG("starting work: book=%s freshStart=%d count=%d lastOffset=%u target=%u fileSize=%u",
  ...
  BPLOG("%s: pages=%d (+%d this tick) lastOffset=%u target=%u "
